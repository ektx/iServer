...
exports.serverStatic = function(req, res, pathname, root, ips) {
	...
	if (fs.existsSync(fileLocation)) {
		...
		if (isDir) {
			showDirecotry(req, res, fileLocation, pathname, ips)
		} else {
...
function showDirecotry(req, res, filePath, pathname, ips) {
	var html = '';
	...
	fs.readdir(filePath, function(err, files) {
		...
		if (ips.join().indexOf(req.ip.replace(/:|f/gi, '')) > -1) {
			html += '<h5><a href="'+pathname+':make" >生成静态页面</a> </h5>';

		}
		html += '<h5>共有 '+ files.length + ' 个文件!</h5>';
		html += '<p class="i-footer">Powered by <a href="https://github.com/ektx/iServer/" target="_blank">iServer</a></p></html>';
		res.write(html)
		res.end()
	})
}
